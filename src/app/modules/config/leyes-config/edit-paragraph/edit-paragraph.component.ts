...
import { ResponseComponent } from 'src/app/modules/main/response/response.component';
...
export class EditParagraphComponent implements OnInit {
  ...
  myControlParagraph= new FormControl();
  optionsParagraphs: any[] = [];
  filteredParagraphsOptions: Observable<any[]>;
  ...
  ngOnInit(): void {
    ...
    this.filteredParagraphsOptions = this.myControlParagraph.valueChanges.pipe(
      startWith(''),
      debounceTime(300),
      map(value => {
        const name = typeof value === 'string' ? value : value?.nombre;
        return name ? this._filterParagraph(name as string) : this.optionsParagraphs.slice();
      }),
    );
  ...
  getParagraphs(){
    this.apiService.getParrafo().subscribe({
      next: response => [
        response = JSON.parse(this.apiService.decrypt(response.message,"private")),
        this.optionsParagraphs = response.result,
        console.log(this.optionsParagraphs)
      ]
    })
  }
  ...
  selectedPar(opt: MatAutocompleteSelectedEvent) {
    console.log(opt);
    this.id_parrafo = opt.option.value.id;
    this.contenido = opt.option.value.contenido; 
    this.myControlOrder.setValue(opt.option.value.orden)
    this.indicator = opt.option.value.indicador
    this.tipo = opt.option.value.tipo.toString()
    this.number = opt.option.value.numero.toString()
    this.nombre = opt.option.value.nombre
    if(opt.option.value.id_articulo !== null){
      this.apiService.getArticleById(opt.option.value.id_articulo).subscribe({
        next: (res) => {
          let artName = JSON.parse(this.apiService.decrypt(res.message, "private"))
          this.myControlArticle.setValue(artName.result[0])
        }     
  ...
  addEvent() {
    ...
    const body = {
      ...
      numero: this.number,
      id_articulo: this.myControlArticle.value.id,
    ...
    this.apiService.editParagraph({data:body})
    .subscribe({
      next: response => {
        this.showSpinner = false;
        this.snackBar.open('Párrafo actualizado correctamente', '', this.config_snack);
        this.resetFields()
        this.getParagraphs()
      },
      error: err => {
  ...
  resetFields() {
    this.myControlParagraph.reset();
    this.indicador = null;
    this.numero = null;
    this.myControlOrder.reset()
  ...
  delete() {
    ...
    .subscribe({
      next: response => {
        this.showSpinner = false;
        this.snackBar.open('Párrafo eliminado correctamente', '', this.config_snack);
        this.resetFields()
        this.getParagraphs()
      },
      error: err => {
  ...
  private _filterParagraph(doc: string): Documents[] {
    const filterValueDoc = doc.toLowerCase();
    return this.optionsParagraphs.filter(option => option.nombre.toLowerCase().includes(filterValueDoc));
  }

  displayParagraph(obj: Article): string {
    return obj && obj.nombre ? obj.nombre : '';
  }
