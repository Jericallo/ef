...
export class NotificationComponent implements OnInit {
  ...
  @ViewChild('notificationContainer') container: ElementRef<HTMLDivElement>;constructor(
    private service: NotificationService,
    private renderer: Renderer2) { }
  private _subscribed: boolean = true;
  private classMap: Map<NotificationType, string>;ngOnInit(): void {
  this.classMap = new Map<NotificationType, string>();
  this.classMap.set(NotificationType.Baja, 'success');
  this.classMap.set(NotificationType.Media, 'warning');
  this.classMap.set(NotificationType.Alta, 'error');
  ...
  this.service.notification
    ...
    let classesToAdd = ['message-box', boxColorClass];classesToAdd.forEach(x => this.renderer.addClass(notificationBox, x));
