...
export class IndexComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  getCumplimientos() {
    let date = new Date(this.sendableDate), y = date.getFullYear(), m = date.getMonth() + 1;

    this.apiService.getAllObligations(m, y).subscribe({
      next: res => {
        const options = { timeZone: 'America/New_York' };
        console.log(res)
        const obligations = res.obligations.map((task: any) => ({ ...task, source: 'obligations' }));
        const overdueObligations = res.overdue.obligations.map((task: any) => ({ ...task, source: 'overdue' }));
        const questionnaire = res.questionnaire ? [{ ...res.questionnaire, source: 'questionnaire' }] : [];

        const allObligations = [...obligations, ...overdueObligations, ...questionnaire];

        const tasks = allObligations.map((task: any) => {
          return {
            id: task.id,
            id_tipo: 1,
  ...
  goToQuestionnaire() {
    let date = new Date(this.sendableDate)
    this.dialog.open(QuestionnaireModalComponent, {
      data: {
